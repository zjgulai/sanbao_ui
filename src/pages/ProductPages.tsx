...
export const AUTOMATION_TEMPLATES: AutomationTemplate[] = [
  { id: 'daily-project-review', title: '每日项目回顾', detail: '汇总工作区里的待处理事项，生成一份本地草稿。', prompt: '汇总当前工作区里的待处理事项，生成一份本地项目回顾草稿。', cadence: '每天 · 18:00', frequency: '每天', time: '18:00', icon: 'file' },
  { id: 'release-readiness', title: '发布前检查', detail: '按一组固定检查项整理结果，保留人工确认步骤。', prompt: '按发布前检查项整理本地结果，并保留人工确认步骤。', cadence: '每个工作日 · 10:00', frequency: '每个工作日', time: '10:00', icon: 'check' },
...
function QoderResearchSidebar({ page, go, pending, onSearch, onWorkspace, collapsed, toggleCollapsed, workspace, initialUserMenu, menuRevision, notify }: Routes & { workspace: WorkspaceConfig; initialUserMenu?: UserAppearancePanel; menuRevision: string; notify: (message: string) => void; page: string; onSearch: () => void; onWorkspace: () => void; collapsed: boolean; toggleCollapsed: () => void }) {
  ...
  useEffect(() => { setUserMenuOpen(!!initialUserMenu); }, [initialUserMenu, menuRevision]);
  return <aside className={`product-sidebar ${collapsed ? 'collapsed' : ''}`}><div className="sidebar-top"><span className="window-dots"><i /><i /><i /></span><IconButton name="panel" label={collapsed ? '展开侧栏' : '收起侧栏'} onClick={toggleCollapsed} /></div><div className="sidebar-content"><div className="mode-tabs"><button className={mode === '编程' ? 'active' : ''} onClick={() => { setMode('编程'); go(6); }}><Icon name="code" size={14} /> 编程</button><button className={mode === '通用' ? 'active' : ''} onClick={() => { setMode('通用'); pending('P01'); }}><Icon name="globe" size={14} /> 通用</button></div><button className="sidebar-row new-task" onClick={() => go(6)}><Icon name="plus" /> 新任务 <kbd>⌘ N</kbd></button><button id="home-search-trigger" className="sidebar-row" onClick={onSearch}><Icon name="search" /> 搜索 <kbd>⌘ G</kbd></button><div className="sidebar-label">工作区 <IconButton name="plus" label="新建工作区" onClick={onWorkspace} /></div><button className="sidebar-row workspace-row" onClick={() => go(6)}><span className="workspace-mark" style={{ background: workspace.color, color: "white" }}>{workspace.name.slice(0, 1).toUpperCase()}</span> {workspace.name} <Icon name="down" size={12} /></button><button className={`sidebar-task ${page === 'session' ? 'active' : ''}`} onClick={() => go(12)}><span className="tiny-dot" /> UI 原型规划</button><div className="sidebar-bottom"><button className={`sidebar-row ${page === 'knowledge' ? 'active' : ''}`} onClick={() => pending('QDR.OBS02.knowledge.empty')}><Icon name="book" /> 知识中心</button><button className={`sidebar-row ${page === 'sites' ? 'active' : ''}`} onClick={() => pending('QDR.OBS03.sites.empty')}><Icon name="globe" /> 站点</button><button className={`sidebar-row ${page === 'automation' ? 'active' : ''}`} onClick={() => go(1)}><Icon name="clock" /> 自动化</button><button className={`sidebar-row ${page === 'extensions' ? 'active' : ''}`} onClick={() => pending('P04')}><Icon name="grid" /> 扩展</button><div className="sidebar-divider" /><button ref={accountRef} className="account-row" aria-label="打开用户菜单" aria-haspopup="menu" aria-expanded={userMenuOpen} onClick={() => setUserMenuOpen(value => !value)}><span className="avatar">三</span><span><strong>三宝设计团队</strong><small>原型虚构账号</small></span><Icon name="settings" size={15} /></button><button className="credit-row" onClick={() => go(16)}><span><Icon name="bolt" size={13} /> 292 Credits</span><span>查看用量 <Icon name="chevron" size={11} /></span></button></div></div><UserAppearanceMenu open={userMenuOpen} key={menuRevision} initialPanel={initialUserMenu ?? "user"} anchorRef={accountRef} onClose={() => setUserMenuOpen(false)} onSettings={() => { setUserMenuOpen(false); pending("QDR.P06.settings.profile.entry"); }} onPending={id => { setUserMenuOpen(false); pending(id); }} onNotice={notify} /></aside>;
}
...
function QoderResearchAutomationPage({ items, view, fixture, onViewChange, onStartCreate, onUseTemplate, onNotice }: AutomationPageProps) {
  ...
  const visible = items.filter(item => mode === '全部模式' || item.mode === mode).slice().sort((a, b) => sort === '名称' ? a.name.localeCompare(b.name, 'zh-CN') : 0);
...
export function AutomationDialog({ close, create, template }: { close: () => void; create: (config: AutomationConfig) => void; template?: AutomationTemplate }) {
  ...
  return <Modal title="新建自动化" className="automation-dialog" onClose={close}><form onSubmit={event => { event.preventDefault(); if (name.trim() && prompt.trim() && time) create({ name: name.trim(), prompt: prompt.trim(), mode, frequency, time, timezone, workspace, model, expires: expire ? expires : '', merge, authorize }); }}><div className="automation-form"><div className="automation-form-main">{template && <p className="automation-template-prefill" role="note">已从“{template.title}”预填名称、任务内容与执行计划；其他字段保留默认本地示例。</p>}<label className="field-label" htmlFor="automation-name">名称</label><input id="automation-name" placeholder="为自动化取一个名字" value={name} onChange={event => setName(event.target.value)} maxLength={80} /><label className="field-label" htmlFor="automation-prompt">任务内容</label><textarea id="automation-prompt" placeholder="描述每次运行时需要完成的任务…" value={prompt} onChange={event => setPrompt(event.target.value)} /><div className="form-mode"><button type="button" className={mode === '编程' ? 'active' : ''} onClick={() => setMode('编程')}><Icon name="code" size={14} /> 编程</button><button type="button" className={mode === '通用' ? 'active' : ''} onClick={() => setMode('通用')}><Icon name="globe" size={14} /> 通用</button></div><div className="form-inline"><label>工作区<select aria-label="自动化工作区" value={workspace} onChange={event => setWorkspace(event.target.value)}><option>paper-plane</option><option>不关联工作区</option></select></label><label>模型<select aria-label="自动化模型" value={model} onChange={event => setModel(event.target.value)}><option>Auto</option><option>示例模型</option></select></label></div><label className="field-label" htmlFor="automation-context">补充上下文</label><input id="automation-context" placeholder="添加本地模拟说明（可选）" /></div><div className="automation-schedule"><label className="field-label" htmlFor="automation-frequency">执行计划</label><select id="automation-frequency" value={frequency} onChange={event => setFrequency(event.target.value)}><option>每天</option><option>每个工作日</option><option>每周</option></select><label className="field-label" htmlFor="automation-time">执行时间</label><input id="automation-time" type="time" required value={time} onChange={event => setTime(event.target.value)} /><label className="field-label" htmlFor="automation-timezone">时区</label><select id="automation-timezone" value={timezone} onChange={event => setTimezone(event.target.value)}><option>Asia/Shanghai</option><option>UTC</option></select><div className="setting-row"><span>设置到期日</span><Toggle label="设置到期日" checked={expire} onChange={setExpire} /></div>{expire && <input type="date" aria-label="到期日期" required value={expires} onChange={event => setExpires(event.target.value)} />}<div className="setting-row"><span>合并任务输出</span><Toggle label="合并任务输出" checked={merge} onChange={setMerge} /></div><p className="field-hint">将多次运行结果保存在同一任务中。</p><div className="setting-row"><span>执行授权</span><Toggle label="执行授权" checked={authorize} onChange={setAuthorize} /></div><p className="field-hint">此处仅演示表单状态，不授予真实权限。</p></div></div><footer className="modal-footer"><span className="local-note">仅保存在本次原型会话</span><button type="button" className="secondary-button" onClick={close}>取消</button><button type="submit" className="primary-button" disabled={!name.trim() || !prompt.trim() || !time || expire && !expires}>创建</button></footer></form></Modal>;
}
...
export function UsageDialog({ close, pending, initialRefreshed = false, onRefreshedChange }: { close: () => void; pending: (group: string) => void; initialRefreshed?: boolean; onRefreshedChange?: (refreshed: boolean) => void }) {
  ...
  return <Modal title="用量" className="usage-dialog" onClose={close}><div className="dialog-body"><div className="usage-title"><h3>套餐额度</h3><button className="text-button" aria-pressed={refreshed} onClick={refresh}>{refreshed ? '示例已刷新' : '刷新'}</button></div><strong className="usage-number">292 <small>/ 300 剩余</small></strong><div className="usage-track"><span /></div><p className="field-hint">已使用 8 Credits · 3%（虚构展示值）</p>{refreshed && <p className="field-hint" role="status">刷新结果仅为 Sanbao 本地原型状态；Qoder 的刷新结果尚未采集。</p>}<div className="sidebar-divider" /><h3>资源包</h3><strong className="usage-number">100 <small>/ 100 剩余</small></strong><div className="usage-links"><button onClick={() => pending('OBS01')}>用量详情 · 待采集</button><button onClick={() => pending('OBS01')}>Rewards · 待采集</button></div></div></Modal>;
}
