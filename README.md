...
当前基线为 `B24-ENTRY-W9`：目录有 **59 个页面组、206 条可直达状态**。其中 **123 条**是已观察状态的本地 fixture，**28 条**已确认入口和 **55 条**静态候选均已落到页面专属的 Sanbao 设计工作面；目录内状态不再以通用研究详情代替实际页面结构。页面覆盖优先于交互精修：所有输入、创建、停止、审批和保存均为本地 fixture，不调用模型、账户、工作区、网络服务或 DSH Host。
