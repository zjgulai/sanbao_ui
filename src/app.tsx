...
export function App() {
  ...
  const referenceTheme = scene && batch21CompatibleReferenceVariants.has(scene.variant) ? 'dark' : undefined;
  ...
  const localFixtureAvailable = available || !!approvalPlanKind || !!artifactWorkbenchEntry || !!executionLifecycleEntry || !!workspaceMultipaneEntry || !!settingsLeafKey || !!taskSearchWorkspaceEntry || !!collaborationEntry || !!agentRuntimeEntry || !!composerWorkbenchEntry || !!taskMonitorEntry || !!entrySystemEntry || !!sessionResilienceEntry || !!settingsExtensionWorkbenchEntry || !!toolResultEntry || !!automationLifecycleEntry || !!sessionInteractionEntry || !!remainingScopeEntry;
  const page = approvalPlanKind || artifactWorkbenchEntry || executionLifecycleEntry || workspaceMultipaneEntry || taskSearchWorkspaceEntry || collaborationEntry || agentRuntimeEntry || composerWorkbenchEntry || taskMonitorEntry || entrySystemEntry || sessionResilienceEntry || toolResultEntry || automationLifecycleEntry || sessionInteractionEntry || remainingScopeEntry ? 'session' : settingsExtensionWorkbenchEntry ? 'extensions' : settingsLeafKey ? 'settings' : initialUserMenu ? 'automation' : initialContextMenu ? 'home' : scene?.page ?? 'home';
  ...
  const sceneKey = `${scene?.page === 'knowledge' ? 'knowledge' : isInstalledExtensionsState(sceneId) ? 'installed-extensions' : isMarketListState(sceneId) ? 'extension-market' : settingsFamily(sceneId) ?? sceneId}-${revision}`;
  const settingsVisible = scene?.page === 'settings' || isMobileSettingsState(sceneId) || !!settingsLeafKey;
