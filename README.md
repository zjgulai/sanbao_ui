...
当前基线为 `B24-VIS-W12`：目录有 **59 个页面组、206 条可直达状态**。其中 **123 条**是已观察状态的本地 fixture，**28 条**已确认入口和 **55 条**静态候选均已落到页面专属的 Sanbao 设计工作面；目录内状态不再以通用研究详情代替实际页面结构。页面覆盖优先于交互精修：所有输入、创建、停止、审批和保存均为本地 fixture，不调用模型、账户、工作区、网络服务或 DSH Host。
...
GitHub Pages 使用独立的公开制品命令：

```sh
pnpm build:pages
```

该命令同样执行上述检查，并将首页、脚本、样式和 SanBao 品牌 SVG 写入 `_site/`。它不复制研究文档、证据截图或已删除的 `website/` 官网源码。

## 接入边界

本仓库是 UI 原型源代码；GitHub Pages 仅发布其静态 UI 原型，不是 DSH Host、模型服务或完整生产交付。它不证明原产品像素一致、Figma 同步、DSH 壳内兼容、iOS 打包或 DMG 发布。接入时应保持页面渲染、状态契约和实际服务调用分离，并以目标 DSH 版本的运行验证为准。
